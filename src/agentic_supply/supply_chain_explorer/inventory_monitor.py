...
from agentic_supply.inventory_assistant.stock_monitoring import get_stock_level, get_safety_level, is_replenishment_needed
from agentic_supply.utilities.config import PRODUCT_NAMES


class InventoryMonitor(CodedTool):
    """
    ...
    async def async_invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Any:
        ...
        replenish_status = is_replenishment_needed(stock_level=stock_level, safety_level=safety_level)
        return f"The stock of {product_name} must {'' if replenish_status else 'not'} be replenished : stock_level={stock_level}, safety_level={safety_level}"
